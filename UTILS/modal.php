...
if ($_POST['action'] == 'searchCliente') {
  if (!empty($_POST['cliente'])) {
    ...
    if ($result > 0) {
      $data = mysqli_fetch_assoc($query);
      echo "Cliente encontrado:", $data['nombre']; // Mensaje en caso de encontrar al cliente
    } else {
      $data = 0;
      echo "Cliente no encontrado"; // Mensaje en caso de no encontrar al cliente
    }
    echo json_encode($data, JSON_UNESCAPED_UNICODE);
  }
  exit;
      ...
      if (!empty($_POST['passActual']) && !empty($_POST['passNuevo'])) {
        ...
        if ($result > 0) {
          ...
          if ($query_update) {
            ...
            header("Refresh:1; URL=../../PRESENTACION/views/salir.php");
          }else {
            $code = '2';
