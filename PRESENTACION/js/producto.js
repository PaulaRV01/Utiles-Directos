  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (data.mensaje == "Cliente no encontrado") {
        ...
        $('#nom_cliente').val(data.cliente.nombre);
        $('#tel_cliente').val(data.cliente.telefono);
        $('#dir_cliente').val(data.cliente.direccion);
      ...
      $.ajax({
          ...
          success: function(response) {
              if (response != 0) {
                  ...
                  }, 3000); // Espera 2 segundos (2000 milisegundos) antes de recargar la página
              } else {
                  console.log('no hay dato');
              }
          },
